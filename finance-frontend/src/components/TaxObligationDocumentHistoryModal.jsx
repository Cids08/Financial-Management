 ...
 *   basically always be the case in practice.
 */
export default function TaxObligationDocumentHistoryModal({ open, onClose, obligation, fetchHistory, onView }) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError('')
    fetchHistory(obligation.tax_id).then((result) => {
      if (cancelled) return
      if (result.success) {
  ...
  const handleView = async (doc) => {
    ...
    // async fetch has already resolved.
    const targetWindow = window.open('', '_blank')
    setViewingId(doc.id)
    const result = await onView(obligation.tax_id, doc.id, targetWindow)
    setViewingId(null)
    if (!result.success) {
      setError(result.message)
