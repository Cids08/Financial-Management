// src/components/ExpenseReceiptUploadModal.jsx
...
/**
 * Modal for attaching a receipt file to an expense.
 *
 * Props:
...
export default function ExpenseReceiptUploadModal({ open, onClose, expense, onUpload }) {
  ...
  const handleSubmit = async () => {
    ...
    if (result?.success) {
      ...
      setError(result?.message || 'Failed to attach the receipt. Please try again.')
    }
  }
      ...
      <div className="space-y-3">
        {expense && (
          <p className="text-xs text-muted">
            Attaching a receipt to <span className="font-medium text-ink">{expense.description}</span>.
          </p>
        )}
